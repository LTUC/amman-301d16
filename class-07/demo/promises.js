...
// pending -> done [fufilled | rejected]

let request = (data) => new Promise((resolve, rejected) => {
  let timer = Math.floor(Math.random() * 1000);
  setTimeout(() => {
    if (true) {
...
const doStuff = async () => {
  try { // replaces .catch
    const task = await request('d - 1'); // await replaces .then 
    console.log(task);
    const task2 = await request(task.timer);
    console.log(task2);
    console.log(await request(task2.timer));
  } catch (error) {
